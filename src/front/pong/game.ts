...
export class game
{
	...
	private	hk				:	BABYLON.HavokPlugin;
	...
	private	is_p2_ia		:	boolean = false;
	private	nbRenders		:	number = 0;
	...
	public get_ball_velocity() : BABYLON.Vector3
	{
	...
	public get_paddle_velo() : number
	{
		return (PDL_VELO_Y);
	}

	public get_paddle_y() : number
	{
		return (this.meshHandler.Paddles.right.mesh.position.y);
	}

	public get_paddle_x() : number
	{
		return (this.meshHandler.Paddles.right.mesh.position.x);
	}
	
	public get_paddle_Y_l() : number
	{
	...
	constructor()
	{
		(async () => {
			...
			this.scene.onBeforeRenderObservable.add((scene) => {
				...
				{
					this.meshHandler.Balls[i].aggr.body.setLinearVelocity((this.meshHandler.Balls[i].aggr.body as any).getLinearVelocity().multiply(new BABYLON.Vector3(1, 1, 0)));
