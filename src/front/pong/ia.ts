...
export class ia
{
	private ball_pos			:	number [];
	private last_ball_pos		:	number [];
	private last_paddle_pos		:	number;
	private ball_velocity		:	BABYLON.Vector3;
	private diff				:	number;
	private paddleX				:	number;
	private paddleY				:	number;
	private supposedPaddleY_new :	number;
	private supposedPaddleY_old :	number;
	private pdl_velo_y			:	number;
	private predictedY			:	number;
	private predictedW			:	number;
	private up					:	boolean;
	...
	constructor(game: game, meshHandler: meshHandler)
	{
