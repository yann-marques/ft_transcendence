...
export async function updateStatus(): Promise<boolean> {
    try {
        const response = await fetch('/api/status', {
            credentials: 'include', // send cookies
        });
...
export async function sendCustomization(jsonData: any) {
    try {
...
export async function uploadAvatar(imgBase64: string) {
    try {
...
async function updateMatchUserOnLobby(data: any, maxPlayerGame) {
    const playersInGame = [];
...
export function setPopupContent(html: string) {
...
export function toggleErrorText(eltId: string, text: string) {
...
export function toggleElt(eltId: string) {
...
export async function updateAuthUserOnLobby() {
    try { 
        ...
        if (response.ok) {
            ...
            let maxPlayerGame = -1;
            if (dataLobby == "1v1") maxPlayerGame = 2;
            if (dataLobby == "1vIA") maxPlayerGame = 1;
            ...
            const buttonPlay = document.getElementById("playPong") as HTMLButtonElement;
            ...
            const buttonAddPlayer = document.getElementById("addPlayer") as HTMLButtonElement;
            ...
            for (const label of Array.from(labelColorPicker)) {
                const input = label.querySelector('input[type="color"]') as HTMLInputElement;
                
                if (!input) continue;
                
                const username = label.getAttribute('data');
                const dataCustoms = JSON.parse(localStorage.getItem('customPlayer'));
                const dataPlayer = dataCustoms.find((user: any) => user.username === username) || null;
...
export async function addWinner(username: string) {
    try {
...
export async function toggleStats(username: string, toogle: boolean) {
